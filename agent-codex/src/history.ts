  ...
  const runtimeInstructions = isYoutubeAnalystRun(input)
    ...
    : isResearchRun(input)
      ? [
          ...
          "Do not finish on a plan or a progress log. Every investigation must end with Markdown sections `## Результат` and `## Источники`, plus at least one verified finding, metric, comparison, or an explicit `Результат не получен` explanation. Create a run directory under /research-runs with the plan, evidence records, audit and report.md, then run the final research validator. Include the exact `/research-runs/.../report.md` path in the answer. Read the completed report and call the governed `create_artifact` tool exactly once with exactly four fields: `title`, a safe `.md` `filename`, `mimeType` set to `text/markdown`, and non-empty inline `content`; do not send `workspacePath` or extra fields. If a source is blocked or stale, finish with a bounded partial result and name the exact limitation; never call a plan or a list of blockers a result, and never claim completion without the report file and successful artifact call.",
      ...
      : isDataControlRun(input)
        ? [
            ...
            "The data-control maintenance cycle is intended to run every six hours via the routine preset `0 */6 * * *`; this cadence does not change the platform's server-side 15-minute minimum. In every cycle, explicitly audit the four one-day HSReplay meta slices: hsreplay_meta_archetypes_legend_eu_1d, hsreplay_meta_top_1000_legend_1d_firecrawl, hsreplay_meta_legend_1d_firecrawl, and hsreplay_meta_diamond_4to1_1d_firecrawl. A state=ok or HTTP 200 response without publication evidence and upstream_freshness.status=fresh is not fresh-only; cached_after_failure, lkg_served, provisional, missing or unknown freshness evidence must remain degraded and be reported.",
        ...
        : isHeartPulseControlRun(input)
          ? [
              ...
              "The HeartPulse control cycle is intended to run every six hours via the routine preset `0 */6 * * *`; this cadence does not change the platform's server-side 15-minute minimum. For the API-to-UI handoff, check these four one-day HSReplay meta slices by exact source id: hsreplay_meta_archetypes_legend_eu_1d, hsreplay_meta_top_1000_legend_1d_firecrawl, hsreplay_meta_legend_1d_firecrawl, and hsreplay_meta_diamond_4to1_1d_firecrawl. Require publication evidence and upstream_freshness.status=fresh before calling a slice fresh-only; state=ok, HTTP 200, cached/LKG or missing/unknown freshness evidence must remain visibly degraded and must not be presented as current data.",
