...
export function instructionsFor(input: RunAgentInput): string {
  ...
  const runtimeInstructions = isYoutubeAnalystRun(input)
    ? [
        ...
        "Use built-in shell only for these commands: run `research-source doctor` once, then run `research-source youtube-transcript --video '<URL>'` once for each accepted video. Do not use YouTube Search, TinyFish, browser, general web search, translation, arbitrary commands, or links found inside captions.",
        ...
        "The deliverable is mandatory: call the governed `create_artifact` tool exactly once with inline content, MIME `text/markdown`, and a safe filename ending in `.md`. Create one combined file for the request, including failure-only requests. After a successful tool result, reply only with a short count of successful and unavailable links; never print the tool JSON.",
      ]
    : isResearchRun(input)
      ? [
