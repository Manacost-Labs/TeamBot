...
### Личное подключение к сервису можно отключить из приложения

На странице «Интеграции» действие «Отключить аккаунт» теперь действительно отзывает OAuth grant
у поддерживаемого сервиса и удаляет локальную связь с зашифрованным refresh-токеном. Если сервис не
подтвердил отзыв (например, временно недоступен), локальный доступ всё равно прекращается, а экран
и аудит явно показывают, что внешний отзыв не подтверждён. Повторное отключение безопасно, а
параллельное переподключение не затирается устаревшим запросом.

### Профиль и команда видны из настроек

В «Настройках рабочего пространства» появился read-only блок профиля: имя, адрес и текущая роль
берутся из защищённого `/api/me`. Администратор может одним переходом открыть управление командой;
обычный участник не видит этот маршрут. При временной ошибке экран показывает понятное состояние и
кнопку повторной проверки, не создавая ложную форму редактирования.

### Ошибка проверки интеграции больше не выглядит как отключённый аккаунт

Экран «Интеграции» теперь явно показывает загрузку и сбой проверки, предлагает повторить запрос и
не принимает повреждённый ответ API за пустой список подключений. Это одинаково работает в списке
интеграций и на странице конкретного сервиса; метаданные подключения проходят безопасную
проекцию без секретов.

### Результаты собраны в одном рабочем разделе

В ManacostTeam появился защищённый раздел «Результаты»: созданные сотрудниками Markdown, текстовые,
PDF, JSON и CSV-файлы находятся через один owner-scoped индекс, с поиском по имени, фильтрами,
постраничной загрузкой и повторной проверкой доступа перед предпросмотром или скачиванием. Потеря
AG-UI envelope больше не скрывает уже сохранённый файл.
