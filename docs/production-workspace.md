...
Расписание создаётся в разговоре через granted routines tools. Управление доступно владельцу на `/routines` (пункт «Расписание»): список, пауза/возобновление, визуальный выбор сотрудника и целевого диалога, редактирование инструкции, cron, timezone и overlap policy, запуск сейчас, последние 20 запусков и удаление. В селекторах показываются имена, а не только внутренние ID; целевой диалог должен быть активным и содержать выбранного сотрудника.
...
- `skip` пропускает новое срабатывание, пока предыдущее выполняется;
- `queue_one` надёжно сохраняет одно срабатывание в очереди, а последующие пересечения схлопывает;
- `allow_overlap` принимает каждое срабатывание отдельным run, но запись в один канонический диалог сериализуется ради целостности истории;
- при `allow_overlap` ожидается только штатный 409 lock conflict: повтор каждые 500 мс, максимум 5 минут, чтение свежей истории выполняется после получения lock; другая ошибка lock не повторяется;
- ручной «запустить сейчас» возвращает conflict при активном/отложенном run для `skip` и `queue_one`, но принимается для `allow_overlap` и при необходимости ждёт тот же lock диалога.

`allow_overlap` означает, что срабатывания не теряются из-за overlap одного расписания, а не одновременную запись двух ответов в один transcript. После 5 минут ожидания занятый диалог завершает run ошибкой. Пересечение с другим расписанием или живым сообщением пользователя также ограничивается единым lock канонического диалога.
...
Для ссылки `https://docs.google.com/document/d/<id>/edit` модель редактора получает только read-only инструменты `read_google_document` и `read_google_document_edit_map`. Документ читается через подключённый OAuth-аккаунт пользователя, служебная оболочка Google Docs отбрасывается, и проверка сравнивает правку с текстом документа, а не с цифрами из URL. Write-tool никогда не передаётся модели.

После принятой правки gateway регистрирует в TeamBot зашифрованное предложение и возвращает ссылку `/editor/google-doc-edits/<id>`. На этой защищённой странице пользователь видит точный diff и нажимает «Сохранить в Google Docs» или «Отменить». Подтверждение доступно только владельцу предложения и повторно проверяет Bot grant, policy и личный OAuth. Все paragraph-local диапазоны применяются одним `batchUpdate` с ревизией, зафиксированной при подготовке. Повторный или параллельный клик не вызывает Google второй раз; устаревшая ревизия ничего не записывает; потерянный ответ после отправки становится `ambiguous` и автоматически не повторяется.

MVP намеренно не предлагает автоматическую запись для многовкладочных документов, таблиц, оглавлений, усечённого чтения, структурных изменений и изменённых абзацев с несколькими форматированными text runs. Исправленный текст при этом остаётся в чате для ручного применения. Содержимое диапазонов хранится только в `encrypted_payload` и удаляется после terminal-state; audit содержит безопасные счётчики и operation id, но не исходный или новый текст.
...
Подробная эксплуатационная последовательность находится в [runbook подтверждаемой записи редактора](runbooks/editor-google-docs.md).
