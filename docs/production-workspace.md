...
Добавление текста в Google Docs и строк в Google Sheets защищено отдельным долговечным контуром at-most-once. Идентичная операция в пределах одного run получает сохранённый operation id: после подтверждённого успеха Google повторно не вызывается, а после обрыва во время отправки операция помечается `ambiguous` и также не повторяется автоматически. В таком случае сначала прочитайте целевой документ или диапазон и разрешите исход вручную. В журнале хранятся только безопасные идентификаторы, хэши и количества строк/ячеек/символов; добавляемое содержимое там не сохраняется.
